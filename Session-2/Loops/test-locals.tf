resource "aws_instance" "Instances_creation1"{
    count =10
    ami= local.ami_id
...
resource "aws_route53_record" "Records1" {
 count=10
  ...
  type    = "A"
  ttl     = 1
  records = var.instance_names[count.index] =="web" ? [aws_instance.Instances_creation1[count.index].public_ip] : [aws_instance.Instances_creation1[count.index].private_ip]
}
